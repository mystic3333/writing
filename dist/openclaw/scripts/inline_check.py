...
from word_lists import (
    ...
    BROKEN_SENTENCE_PATTERNS_CHECK as BROKEN_SENTENCE_PATTERNS,
...
from script_utils import strip_markdown as _strip_md, count_chinese_chars
...
def check_emotion(text, window_text, verbose=False):
    found = find_negative_markers(window_text)
    count = len(found)
...
def check_temperature(text, window_text, verbose=False):
    counts = count_temperature_words(window_text)
    cold, warm, hot, wild = counts["cold"], counts["warm"], counts["hot"], counts["wild"]
    present = []
...
def check_sources(text, h2_sections, verbose=False):
    ...
    for h2_name, h2_text in h2_sections:
        unique_matches = find_real_sources_in_section(h2_text)
...
def check_syntax(text, window_text, verbose=False):
    total = _count_broken(window_text, mode="check")
    return {
        "pass": total >= 1,
        "total_hits": total,
        "patterns": {},
...
def check_banned_words(text, verbose=False):
    ...
    for w in found_words:
        ...
        if positions:
            ...
            items.append({"word": w, "count": len(positions), "snippet": snippet})
    return {
        "pass": len(items) == 0,
        "total": sum(f["count"] for f in items),
        "items": items,
    }
